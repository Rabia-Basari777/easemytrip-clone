...
router.get("/", async function (req, res) {
    try {
        const get = await Hotel.find().lean().exec();
...
router.get("/:id", async function (req, res) {
    try {
        const getById = await Hotel.findById().lean().exec();
...
// get hotels by domestic Id

router.get("/query/:id", async function (req, res) {
    try {
        const getById = await Hotel.find({id: req.params.id}).lean().exec();
        return res.status(200).send(getById);
    }
    catch (err) {
        return res.status(400).send(err.message);
...
// delete the hotels from the database

router.delete("/:id", async function(req, res) {
    try {
        const remove = await Hotel.findByIdAndDelete(req.params.id);
